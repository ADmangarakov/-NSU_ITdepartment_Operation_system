...
int writeWithNewLine(int terminal, size_t *lengthString, char *currentString) {
    ...
    while (' ' != currentString[valueIndex] && 0 != valueIndex) { //searching a space
        --valueIndex;
    }

    if(0 == valueIndex && newLine(terminal, currentString, lengthString, '\n') == EXIT_SUCCESS) { // if no spaces, put \n
        return EXIT_SUCCESS;
    } else if(0 == valueIndex) {
        return EXIT_FAILURE;
    }

    for (size_t i = *lengthString; i != valueIndex; --i) { //erase until space
        if (write(terminal, "\b \b", 3) == ERROR) {
            perror("Error write");
            return EXIT_FAILURE;
        }
    ...
    if (write(terminal, NEW_LINE, strlen(NEW_LINE)) == ERROR) { //put \n
        perror("Error write");
        return EXIT_FAILURE;
    }
...
int editor(int terminal, struct termios *newOption) {
    ...
    while((code = read(terminal, &buffer, COUNT_SYMBS)) > 0) {
...
int main() {
    ...
    if(setNewAttr(terminal, &new_option, &old_option) != EXIT_SUCCESS) {
        close(terminal);
        return EXIT_FAILURE;
    }

    editor(terminal, &new_option);
